import { lockUnlockNode, setIsLockedNode } from "../../../redux/store/project/actions";
import { Node, Project } from "../../../models";
import { Dispatch } from "redux";
import { IsUnsaved } from "../../../helpers";

const OnLockNode = (node: Node, project: Project, isLockedBy: string, dispatch: Dispatch) => {
  if (!IsUnsaved(node)) dispatch(lockUnlockNode(node.id, project.id, !node.isLocked, isLockedBy));
