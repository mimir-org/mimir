...
const DiagramSelectorComponent: FC<Props> = ({ workspaceService, edgeId, nodeId }) => {
  ...
  const onConnect = (params: Connection | Edge) => setElements((els) => {
    const node = workspaceService.functionalNodeMap.get(workspaceService.functionalEdgeMap.get(params.sourceHandle).from);
