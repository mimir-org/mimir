import { Node, RELATION_TYPE, RelationType, Project } from "../../../models/project";
...
const GetTreeViewLegend = (node: Node): Legend[] => {
    ...
    const legends = node.connectors
        .filter(x => x.relationType === RELATION_TYPE.PartOf as RelationType || x.relationType === RELATION_TYPE.Relation as RelationType)
        .map(y => {
...
const GetLegendData = (project: Project, isBlockView: boolean, nodeId: string) => {
    ...
    if (isBlockView) {
        ...
        return legends.filter((value, index, self) => self.map(x => x.color).indexOf(value.color) === index);
    } else {
