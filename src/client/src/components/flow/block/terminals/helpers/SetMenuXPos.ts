...
const SetMenuXPos = (
  ...
  if (hasActiveTerminals && !isParent) return node.width + marginLarge;
  if (hasActiveTerminals && isParent) return parentBlockSize.width + marginLarge;
  if (!hasActiveTerminals && !isParent) return node.width + marginSmall;
  if (!hasActiveTerminals && isParent) return parentBlockSize.width + marginLarge;
};
