...
export function* getUser() {
  try {
    ...
    const user: User = { name: response?.data?.name, email: response?.data?.email, role: response?.data?.role };

    yield put(fetchUserSuccessOrError({ user: user, apiError: null }));
  } catch (error) {
    const apiError: ApiError = { key: fetchUserSuccessOrError.type, errorMessage: error?.message, errorData: null };
