...
namespace Mb.TypeEditor.Services.Services
{
    public class LibraryTypeService : ILibraryTypeService
    {
        ...
        public LibraryTypeService(INodeTypeTerminalTypeRepository nodeTypeTerminalTypeRepository,
            ILibraryRepository libraryRepository, ILibraryTypeRepository libraryTypeComponentRepository,
            IAttributeTypeRepository attributeTypeRepository, ISimpleTypeRepository simpleTypeRepository,
            IMapper mapper, IHttpContextAccessor contextAccessor, IRdsRepository rdsRepository)
        {
        ...
        /// <returns></returns>
        public async Task<IEnumerable<LibraryType>> CreateLibraryTypes(
            ICollection<CreateLibraryType> createLibraryTypes)
        {
        ...
        /// <returns></returns>
        public async Task<T> UpdateLibraryType<T>(string id, CreateLibraryType createLibraryType,
            bool updateMajorVersion, bool updateMinorVersion) where T : class, new()
        {
            ...
            if (double.Parse(existingType.Version, CultureInfo.InvariantCulture) <
                ...
                throw new ModelBuilderInvalidOperationException(
                    $"Not allowed to edit previous {existingType.Version} version. Latest version is {existingTypeVersions[^1].Version}");
        ...
        public async Task DeleteType(string id)
        {
            ...
            if (existingType == null)
                throw new ModelBuilderNotFoundException(
                    $"Could not delete type with id: {id}. The type was not found.");
        ...
        private async Task<IEnumerable<LibraryType>> CreateLibraryTypes(
            ICollection<CreateLibraryType> createLibraryTypes, bool createNewFromExistingVersion)
        {
        ...
        private async Task<T> CreateLibraryType<T>(CreateLibraryType createLibraryType,
            bool createNewFromExistingVersion) where T : class, new()
        {
            if (createLibraryType == null)
                return null;
        ...
        {
            if (string.IsNullOrWhiteSpace(newLibraryType?.Version) ||
                string.IsNullOrWhiteSpace(existingLibraryType?.Version))
